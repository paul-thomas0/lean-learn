...
import { ac, t1, t2, t3, btn, page, container } from "../constants/theme";
    ...
    <div style={page}>
      <div style={container}>
        ...
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ fontSize: 52, marginBottom: 12, filter: "drop-shadow(0 4px 24px rgba(59,130,246,0.2))" }}>TS</div>
          <h1 style={{ fontSize: 32, fontWeight: 900, background: "linear-gradient(135deg, #3b82f6, #a78bfa, #f472b6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", margin: 0, letterSpacing: -0.5 }}>TypeScript Type Trainer</h1>
          <p style={{ color: t2, marginTop: 10, fontSize: 15, fontWeight: 400, lineHeight: 1.5 }}>Master the type system through play</p>
        </div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              {ck.map(k => {
                ...
                return (<button key={k} onClick={() => { const s = new Set(cats); a ? s.delete(k) : s.add(k); setCats(s); }}
                  ...
                  <div style={{ fontSize: 11, marginTop: 3, color: a ? `${c.color}aa` : t3 }}>{n} types</div>
                </button>);
              })}
            </div>
            ...
            <button onClick={() => cats.size > 0 && onStart(cats)} disabled={cats.size === 0}
              style={{
                ...
                background: cats.size > 0 ? "linear-gradient(135deg, #3b82f6, #a78bfa)" : "#1e293b",
                ...
                boxShadow: cats.size > 0 ? "0 4px 24px rgba(59,130,246,0.25)" : "none",
                letterSpacing: 0.3,
              }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {ck.map(k => {
                ...
                return (<button key={k} onClick={() => onLearn(k)}
                  ...
                  <div style={{ fontSize: 11, marginTop: 3, color: t3 }}>{n} types</div>
                </button>);
              })}
            </div>
          </>
