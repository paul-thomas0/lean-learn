...
export default function useQuiz() {
  ...
  useEffect(() => {
    if (ta && tl > 0) { tr.current = setTimeout(() => setTl(t => t - 1), 1000); }
    else if (ta && tl === 0 && mode === M.quiz) { timeout(); }
    return () => clearTimeout(tr.current);
  }, [ta, tl, mode]);

  const timeout = () => {
    if (done) return;
    setDone(true); setTa(false); setStreak(0);
    setHist(h => [...h, { ...cur, ok: false, pick: "⏰ Time's up" }]);
  };
