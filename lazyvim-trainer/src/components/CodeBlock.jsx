        ...
        {ex.before && (
          <div style={{ flex: 1, padding: "14px 16px", background: "#0c1222", borderRight: ex.after ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
            ...
            {ln(ex.before).map((l, i) => (
              <div key={i} style={{ color: t1, whiteSpace: "pre", lineHeight: 1.7 }}>
                {l.split('│').map((seg, j, arr) => (
                  <span key={j}>
                    {seg}
                    {j < arr.length - 1 && <span style={{ color: ac, fontWeight: 900, textShadow: `0 0 8px ${ac}40` }}>▊</span>}
                  </span>
                ))}
              </div>
            ))}
            ...
            {ln(ex.after).map((l, i) => (
              <div key={i} style={{ color: t1, whiteSpace: "pre", lineHeight: 1.7 }}>
                {l.split('│').map((seg, j, arr) => (
                  <span key={j}>
                    {seg}
                    {j < arr.length - 1 && <span style={{ color: ok, fontWeight: 900, textShadow: `0 0 8px ${ok}40` }}>▊</span>}
                  </span>
                ))}
              </div>
            ))}
